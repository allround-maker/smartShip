...
int main (int argc, char** argv){
    ...
    ros::NodeHandle n;
    ros::Publisher chatter_pub = n.advertise<std_msgs::Int32>("docking_point", 10);
    ros::Publisher chatter_pub2 = n.advertise<std_msgs::Float32>("degree", 10);
    ...
    while(j == 0){
            ...
            {   //cout<<"ContourArea : " <<contours.size()<<endl;
                ...
                float degree = ((640 - mu[i].m10/mu[i].m00) * 0.088);
                ...
                {   //cout <<"ContourArea : " << fabs(contourArea(Mat(contours[i]))) << endl;
                    ...
                    {   chatter_pub2.publish(msg2);
                        ...
                        if ((zone_1a <= degree) &&(degree <= zone_1b)){
                            a += 1;

                        }

                        else if ((zone_2a <= degree) &&(degree <= zone_2b)){
                            b += 1;

                        }

                        else if((zone_3a <= degree) &&(degree <= zone_3b)){
                            c += 1;
                        ...
                        if (docking_point > 0)
                        {
                            ...
                            msg.data = docking_point;
                            ROS_INFO("degree : %d", docking_point);

                            n.setParam("/dock_point", docking_point);
