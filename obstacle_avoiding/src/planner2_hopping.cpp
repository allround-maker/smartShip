    ...
    void updateScan(vector<float> rescan){
        ...
        for(int i=0; i<rescan.size(); i++){
            ...
            if(!rescan[i]){
                ...
                point.push_back(inf_to*cos( angle ));
                point.push_back(inf_to*sin( angle ));
            }*/
...
/*
    ...
    float distanceToCollision(float dir, float B){ // calculate collision distance at all data
        ...
        for(int i=start_scan; i<end_scan; i++){
            ...
            if( dis_with_line< B){
                ...
                if(0){
                    cout << "j : " << j << " start / end " << start_scan <<" ~ "<< end_scan << endl; 
    ...
    float scanCollision(float target_yaw){
        ...
        for(int deg = -180; deg<180; deg=deg+1){
            ...
            float edit_theta = 1;
            if(distanceToCollision(0, Breath)>0.1){
                edit_theta = (100.0-(float)abs(deg)/10)/100;
                //cout << "edit_theta = " << edit_theta << endl;
            }
            

            float center_close = collision_data * cos(((float)deg-(float)target_yaw)/180.0*PI);
            ...
            float left_close2 = collision_data * cos(((float)deg-((float)target_yaw-side_dir2))/180.0*PI);
            float right_close2 = collision_data * cos(((float)deg-((float)target_yaw+side_dir2))/180.0*PI);
            //if(left_close <0) left_close=0;
            //if(right_close<0) right_close=0;
            if(!docking){
                if(collision_data>0.5){ //min distance for chosse
...
private:
    float heading_now = 0;
    AxisScan axis_scan;
    float target_heading; // -42 // stardium 110
    float timer_T = 0.2;
    vector<float> test_Vec;
  ...
  Planner()
    {
        ...
        sub_step = n_.subscribe("/run_step", 1, &Planner::updateStep, this);
    ...
    void testscanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
    {
        throw_lidar = throw_lidar*-1;
        ...
        if(throw_lidar ==1){
            ...
            */
            ...
            float distance_ = axis_scan.distanceToCollision(0, Breath);
            ...
            cout << "   distance at dir : "<< distance_<< endl << "    distance at foward : "<< foward_dis_for_break << endl;
    ...
    void updateStep(const ::std_msgs::Int32::ConstPtr& input){
        target_heading = 150;
        run_step = input->data;
        if(run_step==0){
            target_heading = 150;
    ...
    {
        ROS_WARN("target heading %f",target_heading);
        ROS_WARN("run step : %d", run_step);
...
{
  ros::init(argc, argv, "planner2_hopping");     // Initializes Node Name
